...
{
    //when resizing button, text boarder padding is added
    //remove padding in case attempting to size button explicitly
    //and not based an internal calculation (like when resizing text)
    width = mButtonDimensions.width - CPUT_BUTTON_TEXT_BORDER_PADDING_X;
    height = mButtonDimensions.height - CPUT_BUTTON_TEXT_BORDER_PADDING_Y;;
    DEBUG_PRINT(_L("Width %d"), width);
    DEBUG_PRINT(_L("Height %d"), height);
}
...
//--------------------------------------------------------------------------------
void CPUTButton::Resize(int width, int height)
{
    DEBUG_PRINT(_L("Resize(%d, %d)"), width, height);
