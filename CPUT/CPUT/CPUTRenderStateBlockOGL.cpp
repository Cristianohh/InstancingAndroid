...
#else
    ReadProperties( file, L"DepthStencilStateOGL", pDepthStencilDescMap, &mStateDesc.DepthStencilDesc);
    ReadProperties( file, L"RasterizerStateOGL",   pRasterizerDescMap,   &mStateDesc.RasterizerDesc);
    ReadProperties( file, L"RenderTargetBlendStateOGL",   pRenderTargetBlendDescMap,   &mStateDesc.RenderTargetBlendDesc);
#endif
...
#warning "Need to do something with samplers here"
    ...
    mNumSamplers = 0;
    for( ii=0; ii<NUM_SAMPLERS_PER_RENDERSTATE; ii++ )
    {
