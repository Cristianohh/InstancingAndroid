...
#warning "Need to do something with uniform buffers here"
    ...
    if(CPUTFAILED(result))
    {
		DEBUG_PRINT("INITIALIZING GUI FAILED");
        return result;
    }
	DEBUG_PRINT("INITIALIZING GUI OK");
...
void CPUT_OGL::DrawLoadingFrame()
{
    ...
    DEBUG_PRINT(_L("Pre GetController()"));
    CPUTGuiControllerOGL *pGUIController = CPUTGuiControllerOGL::GetController();
    CPUTText *pText = NULL;
    DEBUG_PRINT(_L("Pre CreateText()"));
    pGUIController->CreateText(_L("Just a moment, now loading..."), 999, 0, &pText);
	
    DEBUG_PRINT(_L("Post CreateText()"));
    ...
    DEBUG_PRINT(_L("Post pText ->SetPosition()"));
    ...
    Present();
    DEBUG_PRINT(_L("End of DrawLoadingFrame()"));
}
